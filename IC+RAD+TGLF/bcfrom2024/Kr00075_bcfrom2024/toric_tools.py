...
#make  legend too
    ...
    def powpoynt_ICRF(self,min_indx=-1):
        ...
        max_e = (line_e[0].get_ydata()).max()
        max_min = (line_min[0].get_ydata()).max()
        max_bulk = (line_bulk[0].get_ydata()).max()
        max_ibw = (line_eibw[0].get_ydata()).max()
        
        max_line = max([max_e,max_min,max_bulk,max_ibw])
        ax1.set_ylim(0,1.3*max_line)
...
#        plt.savefig(prefix+'E2Dzeta.eps',format='eps')
    ...
    def toricPlots( self, prefix='',min_indx=-1):
        ...
        * And the 2D parallel electric field contour plot as a png."""
        ...
        plt.tight_layout()
        plt.draw()
        plt.savefig(prefix+'spectrum.pdf',format='pdf',bbox_inches='tight')
	...
	#plt.draw()
        ...
        if (self.mode[:]=='ICRF'):
            ...
            plt.tight_layout()
            plt.draw()
            plt.savefig(prefix+'log10Ezeta2d.pdf',format='pdf',bbox_inches='tight')
            self.plot_2Dfield(component='Eplus',logl=25,im=True)
            plt.tight_layout()
            plt.draw()
            plt.savefig(prefix+'log10Eplus2d.pdf',format='pdf',bbox_inches='tight')
